...
import {
  ...
  Tag,
  Info,
} from "lucide-react"; // Tambah icon Eye, Tag, Info
...
const POS = () => {
  ...
  const [orderType, setOrderType] = useState("DINE_IN"); // DINE_IN atau TAKE_AWAY
  ...
  const handleCheckout = async () => {
    ...
    if (orderType === "DINE_IN" && !tableNumber) {
      toast.warn("Mohon isi Nomor Meja untuk Dine In!");
      return;
    }
    ...
    try {
      const payload = {
        ...
        orderType,
        tableNumber: orderType === "TAKE_AWAY" ? "-" : tableNumber,
      };
        ...
        <header className="px-8 py-6 bg-white border-b border-gray-100 flex items-center justify-between sticky top-0 z-10">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              Temukan makanan favorit pelanggan
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredMenus.map((menu) => (
                <div
                  key={menu.id}
                  className="group bg-white rounded-2xl p-3 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 relative"
                >
                  {/* Image Container */}
                  <div
      ...
      <div className="w-86 bg-white border-l border-gray-200 flex flex-col h-full shadow-2xl z-20">
        ...
        {orderType === "DINE_IN" && (
          <div className="animate-in fade-in slide-in-from-top-2 duration-200">
            <label className="ml-4 mr-4 text-xs font-bold text-gray-400 uppercase tracking-wider">
              Nomor Meja
            </label>
            <input
              type="text"
              placeholder="Contoh: 12"
              className="w-32 mt-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:bg-white outline-none font-bold text-gray-800"
              value={tableNumber}
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id} className="flex gap-4">
                <img
                  src={getImage(item.category.name)}
              ...
              <div className="mt-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                ...
                <p className="text-gray-600 text-sm leading-relaxed">
                  {selectedMenu.description ||
                    "Tidak ada deskripsi tambahan untuk menu ini."}
                </p>
              </div>
            </div>
